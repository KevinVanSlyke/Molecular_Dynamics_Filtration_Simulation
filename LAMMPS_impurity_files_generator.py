...
def LAMMPS_impurity_files_generator(randomSeed, diameterImpurity):
    #randomSeed = [12461,6426357,32578,1247568,124158]
    ...
    dimensions = 2 #Placeholder for when this is eventually generallized to 3D
    xMax = 2000
    xMin = 0
    yMax = 2000
    yMin = 0
    ...
    totalTime = archiveRestartTime+1
    ...
    elif diameterImpurity.is_integer():
        dirName = 'D{0}'.format(int(diameterImpurity),int(filterWidth))
    else:
        dirName = 'D{0}h'.format(int(diameterImpurity),int(filterWidth))
    ...
    if dimensions == 2:
        ...
        sf.write('region    vacuum block {0} {1} {2} {3} {4} {5}    #Region to be filled by gas \n'.format(int(xMin+diameterType[-1]+1), int(xMax-(diameterType[-1] + filterDepth + 1)), int(yMin+diameterType[-1]+1), int(yMax-(diameterType[-1]+yPad+1)), 0, 0))
    ...
    for f in inputFiles:
        ...
        f.write('fix    2 gas wall/reflect ylo {0} yhi {1}    #Fix walls parallel to x axis at y={0} and y={1} to reflect particles \n'.format(int(yMin), int(yMax)))
        if filterWidth > 0:    
            f.write('fix    3 filter setforce 0.0 0.0 0.0  #Fix force on filter to zero \n')
        if dimensions == 2:
    ...
    for f in inputFiles:
        ...
        if dimensions == 2:
            ...
            for i in xrange(iRange):
                if (flagPressFilterFaceOnly == True):
                    if i == iRange-1:
                        xl = i*dx
                        xu = (i+1)*dx-1
                        yl = yMin
                        yu = yMax
                        f.write('region    block{0} block {1} {2} {3} {4} {5} {6} \n'.format(i,xl,xu,yl,yu,zMin,zMax))
                        f.write('group    slice{0} dynamic gas region block{0} every {2} \n'.format(i,j,dynamicTime))
                        if flagPressureFromKineticOnly == True:
                        ...
                        f.write('variable    P{0}x equal -(c_P{0}s[1])/({2}*{3}) \n'.format(i,j,xu-xl+1,yu-yl))
                        f.write('variable    P{0}y equal -(c_P{0}s[2])/({2}*{3}) \n'.format(i,j,xu-xl+1,yu-yl))
                        f.write('variable    P{0} equal (v_P{0}x+v_P{0}y)/2 \n'.format(i,j))
                        if velDumpTime > 0:
                            if atomTypes < 4:
                                f.write('dump    {0} slice{1} custom {2}  dump_'.format(10+i, i, velDumpTime) + dirName + '_slice{0}_'.format(i) + dumpStringDiff + '.lmp vx vy #Dump pressure slice group slice{0} atom data every N={1} timesteps to file dump_'.format(i, velDumpTime) + dumpStringDiff + '_slice{0}_'.format(i) + dirName + '.lmp, include atom: x velocity, y velocity in that order \n')
                            else:
                                f.write('dump    {0} slice{1} custom {2}  dump_'.format(10+i, i, velDumpTime) + dirName + '_slice{0}_'.format(i) + dumpStringDiff + '.lmp vx vy type #Dump pressure slice group slice{0} atom data every N={1} timesteps to file dump_'.format(i, velDumpTime) + dumpStringDiff + '_slice{0}_'.format(i) + dirName + '.lmp, include atom: x velocity, y velocity and mass in that order \n')
                        f.write('\n')  
                else:
                    xl = i*dx
                    xu = (i+1)*dx-1
        ...
        if (flagPressFilterFaceOnly == True):
            f.write('v_P{0} '.format(iRange-1))
        else:
            for i in xrange(0,iRange):
                for j in xrange(0,jRange):
                    f.write('v_P{0} '.format(i,j))
        f.write('\n')
        ...
        if filterWidth > 0:  
            if atomTypes < 4:
                f.write('dump    1 pore custom {0} dump_'.format(dynamicTime) + dirName + '_pore_' + dumpStringDiff + '.lmp id vx    #Dump pore group atom data every N={0} timesteps to file dump_'.format(dynamicTime) + dumpStringDiff + '_pore_' + dirName + '.lmp, include atom: id, x velocity in that order \n')
            else:
                f.write('dump    1 pore custom {0} dump_'.format(dynamicTime) + dirName + '_pore_' + dumpStringDiff + '.lmp id type vx    #Dump pore group atom data every N={0} timesteps to file dump_'.format(dynamicTime) + dumpStringDiff + '_pore_' + dirName + '.lmp, include atom: id, type, x velocity in that order \n')
            
        if tracerTime > 0:
            if atomTypes < 4:
                f.write('dump    2 impurity custom {0} dump_'.format(tracerTime) + dirName + '_tracer_' + dumpStringDiff + '.lmp id x y fx fy    #Dump argon tracer atom data every N={0} timesteps to file dump_'.format(dynamicTime) + dumpStringDiff + '_tracer_' + dirName + '.lmp, include atom: id, scaled x position, scaled y position, x velocity, y velocity, x force, y force in that order \n')
            else:
                f.write('dump    2 impurity custom {0} dump_'.format(tracerTime) + dirName + '_tracer_' + dumpStringDiff + '.lmp id type x y fx fy    #Dump argon and impurity tracer atom data every N={0} timesteps to file dump_'.format(dynamicTime) + dumpStringDiff + '_tracer_' + dirName + '.lmp, include atom: id, mass, scaled x position, scaled y position, x velocity, y velocity, x force, y force in that order \n')
