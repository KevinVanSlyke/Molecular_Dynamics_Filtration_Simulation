...
for dirName in os.listdir(topDir):
    if not (dirName.endswith('.py') or dirName.endswith('.pyc')):
        ...
        rushCores = 4
        mem = 256
        newRushRestartName = 'sbatch_' + dirName + '_restart_' + str(N) + '.sh'
        ...
        r.write('echo "Echo... srun -n $NPROCS lmp_mpi -nocite -screen none -in input_' + dirName + '_restart_' + str(N) + '.lmp -log log_' + dirName + '_restart_' + str(N) + '.lmp" \n')
        r.write('srun -n $NPROCS lmp_mpi -nocite -screen none -in input_' + dirName + '_restart_' + str(N) + '.lmp -log log_' + dirName + '_restart_' + str(N) + '.lmp \n')
 
        r.write('echo "All Done!" \n')
        r.close()


        copy = open('input_' + dirName + '_restart_{0}.lmp'.format(N-1),'r')
