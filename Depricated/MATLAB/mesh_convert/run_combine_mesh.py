...
for dataDir in os.listdir(ensembleDir):
    if os.path.isdir(os.path.join(ensembleDir, dataDir)):
        ...
        shutil.copy2('/user/kgvansly/MATLAB/slurm_combine_mesh.sh',os.getcwd())
        shutil.copy2('/user/kgvansly/MATLAB/combine_mesh.m',os.getcwd())
        myCommand = "sbatch  --job-name="+simString+" --output="+simString+".out --error="+simString+".err slurm_combine_mesh.sh"
        os.system(myCommand)
    else:
        continue
