...
for i = 1 : 1 : size(simList,1)
    ...
    for j = 1 : 1 : size(P_avg,3)
        ...
        if j ~= 2
            ...
            title(strcat('Statistical Pressure at~', region, ' of Filter for Pore Width W=', num2str(w), 'nm and Impurity Diameter D=', num2str(d), 'nm'), 'Interpreter', 'LaTex', 'FontSize', 8 );
            ...
            plot(t, P_plot, '.');
            title(strcat('Average Pressure at~', region, ' of Filter for Pore Width W=', num2str(w), 'nm and Impurity Diameter D=', num2str(d), 'nm'), 'Interpreter', 'LaTex', 'FontSize', 8 );
            xlabel('Time, $t ~ (ns)$','Interpreter','Latex');
            ylabel('Pressure, $P ~ (\frac{mJ}{m^{2}})$','Interpreter','Latex');
            legend('Raw Data');
            %axis([0 t_cutoff 0.9*min(P) 1.1*max(P)]);
        ...
        title(strcat('Fourier Transform of Gas Pressure at~', region, ' for Pore Width W=', num2str(w), 'nm and Impurity Diameter D=', num2str(d), 'nm'), 'Interpreter', 'LaTex', 'FontSize', 8 );
