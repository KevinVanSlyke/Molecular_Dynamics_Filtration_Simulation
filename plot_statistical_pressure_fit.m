...
for i = 1 : 1 : size(simList,1)
    ...
    t = times; %timesteps
    P = P_avg(:,i); %LJ Dimensionless
    P_dev = P_std(:,i); %LJ Dimensionless
    ...
    for j = 1 : 1 : num_peaks
        ...
        P_peaks_dev(j,1) = P_dev(peak_ind(j));
        P_peaks_weight(j,1) = P_dev(peak_ind(j))^(-2);
    end
    ...
    exp_fit_coef = coeffvalues(exp_fit_curve)*epsilon/sigma^(2)*(1/(10^(-3))); %milliJoules/meter^2
    exp_coef_conf_int = confint(exp_fit_curve)*epsilon/sigma^(2)*(1/(10^(-3))); %milliJoules/meter^2
    ...
    exp_fit_line = feval(exp_fit_curve, t)*epsilon/sigma^(2)*(1/(10^(-3))); %milliJoules/meter^2
    
    
    [pow_fit_curve, pow_fit_goodness, pow_fit_output] = fit(t_peaks,P_diff_peaks,'power1','Weights', P_peaks_weight);
    
    pow_fit_coef = coeffvalues(pow_fit_curve)*epsilon/sigma^(2)*(1/(10^(-3))); %milliJoules/meter^2
    pow_coef_conf_int = confint(pow_fit_curve)*epsilon/sigma^(2)*(1/(10^(-3))); %milliJoules/meter^2
    ...
    pow_fit_line = feval(pow_fit_curve, t(2:size(t,1)))*epsilon/sigma^(2)*(1/(10^(-3))); %milliJoules/meter^2
    %t_cutoff = 10*tau_fit;
    
    %cd('/home/Kevin/Documents/Dust_Data/Molecular/.../Figures');
    fit_data{i,:} = {simString, tau_fit, tau_stddev, exp_rsquare, exp_adjrsquare, exp_rmse, beta_fit, beta_stddev, pow_rsquare, pow_adjrsquare, pow_rmse, fundFreq};
    ...
    P_plot_diff_peaks = P_diff_peaks*epsilon/sigma^(2)*(1/(10^(-3))); %milliJoules/meter^2
    P_plot_thermal = P_thermal*epsilon/sigma^(2)*(1/(10^(-3))); %milliJoules/meter^2

    ax1 = axes('Position',[.1 .1 .5 .8],'Visible','off');
    %errorbar(t,P_plot, P_plot_dev, '.');
    plot(t, P_plot, '.');
    hold on;
    errorbar(t_peaks, (P_plot_diff_peaks+P_plot_thermal), P_plot_peaks_dev, 'o');
    hold on;
    plot(t, (exp_fit_line+P_plot_thermal), t(2:size(t,1)), (pow_fit_line+P_plot_thermal));    %plot(t, P, '.', t_peaks, (P_diff_peaks+P_thermal), 'ro', t, (exp_fit_line+P_thermal), t(2:size(t,1)), (pow_fit_line+P_thermal));
    title(['Statistical Pressure at Front of Filter for Pore Width W=' num2str(w) 'nm and Impurity Diameter D=' num2str(d) 'nm'], 'Interpreter', 'LaTex', 'FontSize', 8 );
    ...
    print(strcat('Statistical_Pressure_vs_Time_',simString), '-dpng');
    close(fig);
    ...
    print(strcat('Statistical_FFT_',simString), '-dpng');
    close(fig);
end
