function [] = scratch_Na_Ni_Vcm_mesh_multi_H()%ensembleDir, figDir, doPlot)
...
fileName = strcat('Na_Ni_mesh_hist_3multi_H_FL1SL2O_Reflective_Scratch');
...
tiledlayout(size(simList,1)*2,6, 'Padding', 'tight', 'TileSpacing', 'tight');
titles = {'~\qquad~\qquad~(a)'; '~(b)'; '\qquad~\qquad~(c)'; '~(d)'; '\qquad~\qquad~(e)'; '~(f)'; '~\qquad~\qquad~(g)'; '~(h)'; '~\qquad~\qquad~(i)';'~(j)'; '~\qquad~\qquad~(k)';'~(l)'; '\qquad~\qquad~(m)';'~(n)'; '~\qquad~\qquad~(o)';'~(p)'; '~\qquad~\qquad~(q)';'~(r)'; '~\qquad~\qquad~(s)';'~(t)'};
rootDir = ensembleDir;
