function [SL1AFitCurve,SL1AFitGoodness,SL1IFitCurve,SL1IFitGoodness] = Phi_vel_SL_mean_trends_plot(phiStatData,dataLabels,parVar,parCols,figDir)
...
shift = 3;
sortedPhiStatData = sortrows(phiStatData, parCols); %Sort with 'D' most rapidly varying
parVals = sortedPhiStatData(:,parCols);
meanSL1A = sortedPhiStatData(:,3+shift);
meanSL2A = sortedPhiStatData(:,4+shift);
stdSL1A = sortedPhiStatData(:,5+shift);
stdSL2A = sortedPhiStatData(:,6+shift);
weightSL1A = 1./stdSL1A.^2;
weightSL2A = 1./stdSL2A.^2;

if strcmp(parVar,"D")
    meanSL1I = [sortedPhiStatData(1,3+shift); sortedPhiStatData(2:size(parVals),7+shift)];
    meanSL2I = [sortedPhiStatData(1,4+shift); sortedPhiStatData(2:size(parVals),8+shift)];
    stdSL1I = [sortedPhiStatData(1,5+shift); sortedPhiStatData(2:size(parVals),9+shift)];
    stdSL2I = [sortedPhiStatData(1,6+shift); sortedPhiStatData(2:size(parVals),10+shift)];
else
    meanSL1I = sortedPhiStatData(:,7+shift);
    meanSL2I = sortedPhiStatData(:,8+shift);
    stdSL1I = sortedPhiStatData(:,9+shift);
    stdSL2I = sortedPhiStatData(:,10+shift);
end
