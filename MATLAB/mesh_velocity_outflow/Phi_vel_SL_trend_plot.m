function [] = Phi_vel_SL_mean(meanPhi,dataLabels,figDir)
...
dVals = sortedMeanPhi(1:nSVals:size(sortedMeanPhi,1),2);
sVals = sortedMeanPhi(1:nSVals,1);
...
if nSVals > 1
    for i=1:1:nDVals
        ...
        hold on
        plot(sVals,sortedMeanPhi((1:nSVals)+(i-1)*nSVals,5),'-r');
        plot(sVals,sortedMeanPhi((1:nSVals)+(i-1)*nSVals,6),'--r');
        plot(sVals,sortedMeanPhi((1:nSVals)+(i-1)*nSVals,9),'-k');
        plot(sVals,sortedMeanPhi((1:nSVals)+(i-1)*nSVals,10),'--k');
        ...
        if dVals(i) > 1
            ...
            hold on
            plot(sVals,sortedMeanPhi((1:nSVals)+(i-1)*nSVals,15),'-r');
            plot(sVals,sortedMeanPhi((1:nSVals)+(i-1)*nSVals,16),'--r');
            plot(sVals,sortedMeanPhi((1:nSVals)+(i-1)*nSVals,19),'-k');
            plot(sVals,sortedMeanPhi((1:nSVals)+(i-1)*nSVals,20),'--k');
            legend([regionLabels(5-3),regionLabels(6-3),regionLabels(9-3),regionLabels(10-3)]);
...
if nDVals > 1
    ...
    for i=1:1:nSVals
        ...
        hold on
        plot(dVals,sortedMeanPhi((1:nDVals)+(i-1)*nDVals,5),'-r');
        plot(dVals,sortedMeanPhi((1:nDVals)+(i-1)*nDVals,6),'--r');
        plot(dVals,sortedMeanPhi((1:nDVals)+(i-1)*nDVals,9),'-k');
        plot(dVals,sortedMeanPhi((1:nDVals)+(i-1)*nDVals,10),'--k');
        legend([regionLabels(5-3),regionLabels(6-3),regionLabels(9-3),regionLabels(10-3)]);
        ylabel('Argon Outflow Angle, $\Phi_A~(Deg.)$','Interpreter','latex');
        ...
        hold on
        plot(dVals,sortedMeanPhi((1:nDVals)+(i-1)*nDVals,15),'-r');
        plot(dVals,sortedMeanPhi((1:nDVals)+(i-1)*nDVals,16),'--r');
        plot(dVals,sortedMeanPhi((1:nDVals)+(i-1)*nDVals,19),'-k');
        plot(dVals,sortedMeanPhi((1:nDVals)+(i-1)*nDVals,20),'--k');
        legend([regionLabels(5-3),regionLabels(6-3),regionLabels(9-3),regionLabels(10-3)]);
        ylabel('Impurity Outflow Angle, $\Phi_I~(Deg.)$','Interpreter','latex');
        xlabel('Impurity Diameter, $D~(r^*)$','Interpreter','latex');
        %             nameTag = strcat('Argon_Upper_Diameter_Trend_',num2str(sVals(i,1)),'L_',regime);
