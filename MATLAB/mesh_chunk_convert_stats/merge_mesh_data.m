...
for n = 1 : 1 : nFiles
    if endsWith(dataDirList(n).name(), 'mesh.mat') % && ~startsWith(dataDirList(n).name(), 'Mesh')
        ...
        if startsWith(dataDirList(n).name(), 'argon_vcm')
            if trialID == 1
                load(dataDirList(n).name(),'x','y');
                %Will have to make robust for uneven length trials
%                 nTimes = size(t,1);
            end
            load(dataDirList(n).name(),'meshData');
            uA(:,:,1:nTimes,trialID) = meshData(:,:,1:nTimes,1);
            vA(:,:,1:nTimes,trialID) = meshData(:,:,1:nTimes,2);
            clear meshData
        elseif startsWith(dataDirList(n).name(), 'impurity_vcm')
            load(dataDirList(n).name(),'meshData');
            uI(:,:,1:nTimes,trialID) = meshData(:,:,1:nTimes,1);
            vI(:,:,1:nTimes,trialID) = meshData(:,:,1:nTimes,2);
            clear meshData
        elseif startsWith(dataDirList(n).name(), 'argon_count')
            load(dataDirList(n).name(),'meshData');
            countA(:,:,1:nTimes,trialID) = meshData(:,:,1:nTimes,1);
            clear meshData
        elseif startsWith(dataDirList(n).name(), 'impurity_count')
            load(dataDirList(n).name(),'meshData');
            countI(:,:,1:nTimes,trialID) = meshData(:,:,1:nTimes,1);
            clear meshData
        elseif startsWith(dataDirList(n).name(), 'argon_temp')
            load(dataDirList(n).name(),'meshData');
            tempA(:,:,1:nTimes,trialID) = meshData(:,:,1:nTimes,1);
            clear meshData
        elseif startsWith(dataDirList(n).name(), 'impurity_temp')
            load(dataDirList(n).name(),'meshData');
            tempI(:,:,1:nTimes,trialID) = meshData(:,:,1:nTimes,1);
            clear meshData
        elseif startsWith(dataDirList(n).name(), 'argon_internalTemp')
            load(dataDirList(n).name(),'meshData');
            internalTempA(:,:,1:nTimes,trialID) = meshData(:,:,1:nTimes,1);
            clear meshData
        elseif startsWith(dataDirList(n).name(), 'impurity_internalTemp')
            load(dataDirList(n).name(),'meshData');
            internalTempI(:,:,1:nTimes,trialID) = meshData(:,:,1:nTimes,1);
            clear meshData
        end
