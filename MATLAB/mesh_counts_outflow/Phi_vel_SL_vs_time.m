function [regionLabels,phiAValues,phiIValues] = Phi_vel_SL_vs_time(simString,x,y,t,countA,uA,vA,countI,uI,vI,figDir)
...
if nOrifice == 3
    ...
    upDiff = nCells-(orificeIndices(1,2,3)-nSpacing/2);
    lowDiff = (orificeIndices(2,2,2)+nSpacing/2)-1;
    phiRegions(:,:,1) = [orificeIndices(2,1,3)+1, orificeIndices(1,2,3)-nSpacing/2; nCells, nCells];
    if lowDiff > upDiff
        phiRegions(:,:,2) = [orificeIndices(2,1,2)+1, orificeIndices(2,2,2)+nSpacing/2-upDiff; nCells, orificeIndices(2,2,2)+nSpacing/2];
    else
        phiRegions(:,:,2) = [orificeIndices(2,1,2)+1, 1; nCells, orificeIndices(2,2,2)+nSpacing/2];
    end
end
if nOrifice == 4
    nWidth = orificeIndices(2,2,1)-orificeIndices(1,2,1)+1; %Width of orifice
    ...
    %nShift = ... %Registry shift of layer 2
    phiRegions(:,:,1) = [orificeIndices(2,1,4)+1, orificeIndices(1,2,4)-nSpacing/2; nCells, nCells];
    phiRegions(:,:,2) = [orificeIndices(2,1,3)+1, 1; nCells, orificeIndices(2,2,3)+nSpacing/2];
end
...
%         xlabel('Time $(t^*)$','Interpreter','latex');
    ...
    fileName = strcat("Phi_vel_SL_Time_",simString);
