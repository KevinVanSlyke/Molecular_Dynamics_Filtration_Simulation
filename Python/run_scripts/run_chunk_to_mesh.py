...
for dataDir in os.listdir(ensembleDir):
    if not (dataDir.endswith('.py') or dataDir.endswith('.pyc') or dataDir.endswith('.sh') or dataDir.endswith('.m')):
        ...
        for files in os.listdir(os.getcwd()):
            if files.startswith('argon_vcm') and files.endswith('.chunk'):
        ...
        if debug:
            ...
            print('Setting nTrials = 3 for debugging')
            nTrials = 3
        ...
        for trialIndex in range(nTrials):
            ...
            aCommand = "sbatch  --job-name="+jobName+" --output="+jobName+".out --error="+jobName+".err --export=simString="+simString+",atomType="+atomType+",trialIndex="+str(trialIndex)+",nBinsX="+str(nBinsX)+",nBinsY="+str(nBinsY)+",debug="+str(debug)+" slurm_chunk_to_mesh.sh" 
            print(aCommand)
            os.system(aCommand)
            if not simString[1].startswith('1'):
                atomType = 'impurity'
                jobName = simString+'_'+atomType+'_'+str(trialIndex)+'T'
                iCommand = "sbatch  --job-name="+jobName+" --output="+jobName+".out --error="+jobName+".err --export=simString="+simString+",atomType="+atomType+",trialIndex="+str(trialIndex)+",nBinsX="+str(nBinsX)+",nBinsY="+str(nBinsY)+",debug="+str(debug)+" slurm_chunk_to_mesh.sh"
                print(iCommand)
                os.system(iCommand)
        if debug:
            break
