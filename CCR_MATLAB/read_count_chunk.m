function [countChunkData] = read_count_chunk()
%UNTITLED Summary of this function goes here
%   Detailed explanation goes here
dirParts = strsplit(pwd,'/');
simDir = dirParts(end);
parString = simDir{1,1};
chunkFile = strcat('avg_count_chunks_',parString,'_0T_r0.lmp');
outputFile = strcat('count_data_',parString,'_0T_r0.mat');
try
...
countChunkData = zeros(nSteps,nChunks,nVars);
while feof(fid) == 0
    chunkLine = fgetl(fid);
    ...
    if isNum == 1
        if strcmp(chunkWords{1,2},num2str(nChunks))
            ...
            for i=1:1:nVars
                var = varWords{1,i};
                countChunkData(index,chunkID,i) = str2double(var);
            end
        end
    end
end
...
save(outputFile, 'countChunkData');
end
